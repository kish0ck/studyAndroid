...
class User1 constructor(name : String) { //클래스 네이밍은 대문자로 시작한다
    ...
    init { // 클래스가 생성될 때 ( 클래스를 통해서 객체를 만들떄) 호출 된다
        println(name)
        userName = name
    }
}
...
class User7 constructor(name : String){
    ...
    constructor(name: String, age : Int) : this(name){//User7
        println("second")
        this.age = age
    }
    // 부생성자는 복수개 존재 할 수 있다
...
//  - 초기화 블록에서 초기화를 보장해주기 때문 -> 클래스가 생성될 때 초기화블록은 무조건 실행된다
//  - 초기화 블록에 없는 속성은 선언할때 초기화 해줘야 한다
