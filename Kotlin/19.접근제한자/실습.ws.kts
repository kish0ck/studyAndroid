...
class Numbers(var number: Int = 10){
    fun changeNumber(){
        this.number = 100 // Numbers 라는 스코프 안에 존재하기 때문에 number에 접근/변경할 수 있다
    }
...
//println(numbers.number) -> Numbers밖에 존재하기 떄문에 number에 접근/변경할 수 없다
//numbers.number = 100 -> Numbers밖에 존재하기 떄문에 number에 접근/변경할 수 없다
